...
echo "  6. 按 plan/build/qa gate 流程执行，单元测试与 required QA Gate 通过后才可改 passes"
echo "  7. 明确小 bug 可先运行：python3 ${WORKSPACE_PREFIX}scripts/quick_fix_classifier.py --target-dir . --prompt '<bug 描述>'"
echo "  8. 如需手动更新运行时：python3 ${WORKSPACE_PREFIX}scripts/update_runtime.py --target-dir . --dry-run"
echo "  9. 标准任务收口：python3 ${WORKSPACE_PREFIX}scripts/session_close.py --target-dir . --feature-id <task-id> --outcome pass"
echo " 10. quick-fix 收口：python3 ${WORKSPACE_PREFIX}scripts/session_close.py --target-dir . --quick-fix --title '<bug 标题>' --outcome pass --note '<验证命令和结果>'"
echo " 11. 自动续跑下个任务：python3 ${WORKSPACE_PREFIX}scripts/task_switch.py continue --target-dir ."
echo " 12. git commit / git push"
echo ""
