...
| `quick-fix` | “修一个明确小 bug”, “quick fix 这个报错” | 先运行 quick-fix 分类器；high 走轻量修复，medium 询问用户，low 回到 sprint | 代码改动 + 定向验证 + quick close 日志 |
...
- `.harness/task-harness/tasks/`：v3 权威任务目录，新任务默认按批次写入 `<batch-id>/<display-id>-<中文标题>-<hash>.json`
